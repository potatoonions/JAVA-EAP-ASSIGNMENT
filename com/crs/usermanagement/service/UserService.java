package com.crs.usermanagement.service;
...
import com.crs.usermanagement.util.PasswordUtil;
...
public class UserService {
    ...
    public User createUserAccount(String name, String email, String rawPassword, User.Role role) {
        ...
        User user = new User(0, name, email, hashedPassword, resolvedRole, User.Status.ACTIVE);
    ...
    public void permanentlyDeactivateAccount(int userId) {
        ...
        SessionManager.getInstance().invalidateForUser(userId);
        LOG.warning("UserService.permanentlyDeactivateAccount — userId=" + userId + " permanently deactivated.");
    }
    ...
    public User recoverAccount(String email) {
        ...
        if (user.getStatus() == User.Status.ACTIVE)
            throw new IllegalStateException("Account is already active: " + email);
               
        user.clearFailedAttempts();
        userDAO.activateUser(user.getUserId());
        LOG.info("UserService.recoverAccount — userId=" + user.getUserId() + " restored to ACTIVE.");
    ...
    public String resetPassword(String email) {
        ...
        System.out.printf("[EMAIL SIMULATION] Password reset link sent to %s%n" + "  Link: https://crs.university.edu/reset-password?token=%s%n"+ "  Expires: %s%n", email, token, expiry);
    ...
    public void completePasswordReset(String token, String newRawPassword) {
        ...
        LOG.info("UserService.completePasswordReset — password updated for userId=" + user.getUserId());
    }
    ...
    public void assignRole(User requestingUser, int targetUserId, String newRole) {
        ...
        LOG.info(String.format("UserService.assignRole — admin %d set userId=%d to role=%s", requestingUser.getUserId(), targetUserId, newRole));
    }
