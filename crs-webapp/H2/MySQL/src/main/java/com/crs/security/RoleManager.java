...
public class RoleManager {
    ...
    static {
        ...
        // INSTRUCTOR — academic operations only
    ...
    public void assignRole(User user, String newRole) {
        ...
        try {
            ...
            user.setRole(role);
            LOG.info(String.format("RoleManager.assignRole — userId=%d %s → %s",
                user.getUserId(), oldRole, role));
        } catch (IllegalArgumentException e) {
            throw new IllegalArgumentException(
                "Unknown role: '" + newRole + "'. Valid roles: "
    ...
    public void assignRole(User user, User.Role newRole) {
        ...
        LOG.info(String.format("RoleManager.assignRole — userId=%d %s → %s",
            user.getUserId(), oldRole, newRole));
    }
    ...
    public boolean checkPermission(User user, String action) {
        ...
        LOG.fine(String.format("RoleManager.checkPermission — userId=%d role=%s action=%s → %b",
            user.getUserId(), user.getRole(), action, permitted));
        return permitted;
    }
