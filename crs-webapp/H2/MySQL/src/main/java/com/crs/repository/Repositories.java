...
public interface ScheduledNotificationRepository
        extends JpaRepository<ScheduledNotificationEntity, String> {
    ...
    /** All pending notifications for a specific recipient. */
    ...
    /** Cancel all pending notifications for a recipient. */
    @Query("""
...
//  StudentMilestoneRepository
@Repository
public interface StudentMilestoneRepository
        extends JpaRepository<StudentMilestoneEntity, Long> {

    List<StudentMilestoneEntity> findByStudentUserId(String studentId);

    List<StudentMilestoneEntity> findByStudentUserIdAndCompleted(
        String studentId, boolean completed);

    @Query("""
        SELECT m FROM StudentMilestoneEntity m
        WHERE m.student.userId = :studentId
          AND m.completed      = false
          AND m.dueDate       <= :before
        ORDER BY m.dueDate ASC
        """)
    List<StudentMilestoneEntity> findOverdueMilestones(
        @Param("studentId") String studentId,
